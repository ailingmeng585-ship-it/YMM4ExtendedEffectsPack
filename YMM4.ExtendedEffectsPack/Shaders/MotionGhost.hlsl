...
float4 main(float4 pos:SV_POSITION, float4 posScene:SCENE_POSITION, float4 uv0:TEXCOORD0):SV_Target {
    float2 uv=uv0.xy; float2 dir=float2(1,0.15)*uSpread*0.001;
    float4 src=samp(uv); float3 acc=src.rgb; float wsum=1;
    [unroll] for (int i=1;i<=6;i++) {
        float fall=exp(-i*0.14*uSpeed);
        float sc=1+(uSize/100)*(i/max(uCount,1));
        float2 u=(uv-0.5)/sc+0.5-dir*i*1.6;
        acc+=float3(samp(u+dir).r, samp(u).g, samp(u-dir).b)*fall*uMix;
        wsum+=fall*uMix;
    }
    return float4(acc/wsum,1);
}