...
float4 main(float4 pos:SV_POSITION, float4 posScene:SCENE_POSITION, float4 uv0:TEXCOORD0):SV_Target {
    ...
    float2 px=float2(0.002,0.002)*uSize;
    float e=abs(lum(samp(uv+float2(px.x,0)).rgb)-lum(samp(uv-float2(px.x,0)).rgb))
           +abs(lum(samp(uv+float2(0,px.y)).rgb)-lum(samp(uv-float2(0,px.y)).rgb));
    e=smoothstep(0.05,0.7,e);
    float3 neon=lerp(float3(uColorR,uColorG,uColorB), hsv2rgb(float3(frac(uTime*uSpeed*0.15),0.85,1)), 0.55);
    return float4(src.rgb*uMix + neon*e*uStrength*1.6, 1);
}