...
/// <summary>
/// Bounded, processor-local temporal history. Never retains a live effect graph as a frame.
...
internal sealed class FrameHistory : IDisposable
{
    ...
    public ID2D1Image? Update(ID2D1Image input, Vector4 bounds, int frame, int requestedDelay)
    {
        ...
        if (bounds != rect || requestedDelay != delay || (previous >= 0 && frame != previous && frame != previous + 1))
        {
            ...
            // 16-bit float RGBA snapshots: quality reduction is confined to the ghost image.
            scale = (float)Math.Min(1, Math.Sqrt(BudgetBytes / ((delay + 1.0) * bounds.Z * bounds.W * 8)));
